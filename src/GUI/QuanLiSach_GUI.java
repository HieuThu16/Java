...
public class QuanLiSach_GUI extends JFrame {
	//private NhapXuatExcel nhapXuatExcel = new NhapXuatExcel(); 
	...
	private JComboBox<String> comboBox_maNXB1; 
	private JLabel label_hinhAnh ;
	private JComboBox<String> comboBox_maTheLoai1;
	private JComboBox<String> comboBox_maTacGia1;
	private JComboBox<String> suggestionComboBox;
	private DefaultListModel<String> suggestionListModel;
	private int numberOfSuggestionsToShow;
	...
	public static void main(String[] args) {
	    EventQueue.invokeLater(new Runnable() {
	        public void run() {
	            try {
	                QuanLiSach_GUI frame = new QuanLiSach_GUI();
	                frame.setVisible(true);
	                // Load dữ liệu từ cơ sở dữ liệu khi ứng dụng khởi chạy
	                QuanLiSach_GUI.loadTableData(table);
	            } catch (Exception e) {
	                e.printStackTrace();
	            }
	        }
	...
	public QuanLiSach_GUI() {
		setDefaultCloseOperation(JFrame.EXIT_ON_CLOSE);
		setBounds(200, 30, 1258, 800);
		...
		loadDataIntoComboBox_maTheLoai(comboBox_maTheLoai);
		...
		loadDataIntoComboBox_maTacGia(comboBox_maTacGia);
		...
		table.addMouseListener(new MouseAdapter() {
		    public void mouseClicked(MouseEvent e) {
		        ...
		        if (selectedRow != -1) {
		            ...
		            // Lấy đường dẫn hình ảnh từ dữ liệu hàng được chọn
		...
		textField_maSach1.setBounds(623, 80, 203, 32);
		contentPane.add(textField_maSach1);
		...
		comboBox_maNXB1 = new JComboBox<String>();
		comboBox_maNXB1.setFont(new Font("Tahoma", Font.BOLD, 16));
		comboBox_maNXB1.setBounds(1016, 71, 193, 34);
		contentPane.add(comboBox_maNXB1);
		comboBox_maNXB1.addItem("Tất cả");
		loadDataIntoComboBox_maNhaXuatBan(comboBox_maNXB1);
		...
		comboBox_maTheLoai1 = new JComboBox<String>();
		comboBox_maTheLoai1.setFont(new Font("Tahoma", Font.BOLD, 16));
		comboBox_maTheLoai1.setBounds(623, 122, 203, 33);
		contentPane.add(comboBox_maTheLoai1);
		comboBox_maTheLoai1.addItem("Tất cả");
		loadDataIntoComboBox_maTheLoai(comboBox_maTheLoai1);
		...
		contentPane.add(label_maTacGia1);
		
		
		comboBox_maTacGia1 = new JComboBox<String>();
		comboBox_maTacGia1.setFont(new Font("Tahoma", Font.BOLD, 16));
		comboBox_maTacGia1.setBounds(1016, 115, 193, 30);
		contentPane.add(comboBox_maTacGia1);
		comboBox_maTacGia1.addItem("Tất cả");
		loadDataIntoComboBox_maTacGia(comboBox_maTacGia1);
		...
		contentPane.add(textField_tenSach1);
		...
		textField_tu.setBounds(731, 236, 167, 28);
		    ...
		    public void actionPerformed(ActionEvent e) {
		        ...
		        if (selectedRow != -1) {
		            try {
		                ...
		                boolean updated = Sach_DAO.updateSach(sach);
		                if (updated) {
		...
		button_xoa.addActionListener(new ActionListener() {
		    ...
		    public void actionPerformed(ActionEvent e) {
		        ...
		        boolean deleted = Sach_DAO.deleteSach(maSach); // Gọi phương thức deleteSach để xóa sách
		        if (deleted) {
		...
		button_lamMoi.setBounds(534, 723, 140, 33);
		...
		contentPane.add(button_xuatExcel);
		...
		contentPane.add(button_nhapExcel);
		 ImageIcon icon_thongKe1 = new ImageIcon("src\\icon\\excel.png");
		    ...
		    contentPane.add(button_themAnh);
...
//		
	...
	private void updateSuggestions() throws SQLException {
	    ...
	    // Xóa các mục hiện tại trong danh sách gợi ý
	  //  suggestionListModel.clear();

	    // Thực hiện truy vấn để lấy danh sách gợi ý từ cơ sở dữ liệu hoặc nguồn dữ liệu khác
	    if (!input.isEmpty()) {
	        // Giả sử rằng bạn có một phương thức để truy vấn gợi ý từ cơ sở dữ liệu
	        List<String> suggestions = Sach_DAO.getSimilarBookTitles(input);
	...
	private void searchAndUpdate() {
		    String valueMaSach = textField_maSach1.getText() ;// Lấy giá trị từ JTextField và loại bỏ các khoảng trắng
	        String valueMaNXB = (String) comboBox_maNXB1.getSelectedItem();
	        String valueMaTheLoai = (String) comboBox_maTheLoai1.getSelectedItem();
	        String valueMaTacGia = (String) comboBox_maTacGia1.getSelectedItem();
	        ...
	        if (minDonGia > maxDonGia) {
	            ...
	            searchAndDisplayFromDatabase(valueMaSach, valueMaNXB,
	            		valueMaTheLoai, valueMaTacGia, valueTenSach,
	 ...
	 public static void loadTableData(JTable table) {
	        DefaultTableModel model = (DefaultTableModel) table.getModel();
	        model.setRowCount(0); 
	        ArrayList<Sach_DTO> sachList = Sach_DAO.getAllBooks();
	 ...
	 public static void loadDataIntoComboBox_maTacGia(JComboBox<String> tenComboBox) {
	        TacGia_DAO tacGiaDAO = new TacGia_DAO(connection); 
	        ArrayList<TacGia_DTO> authors = tacGiaDAO.getAllAuthors(); 
	        for (TacGia_DTO author : authors) {
	        	tenComboBox.addItem(author.getMaTacGia()); 
	        }
	 }
	 public static void loadDataIntoComboBox_maNhaXuatBan(JComboBox<String> tenComboBox) {
		   // comboBox_maNXB.removeAllItems(); // Xóa tất cả các mục hiện có trong combobox
		    NhaXuatBan_DAO nhaXuatBanDAO = new NhaXuatBan_DAO(connection); // Tạo một đối tượng DAO
		    ArrayList<NhaXuatBan_DTO> publishers = nhaXuatBanDAO.getAllPublishers(); // Lấy danh sách nhà xuất bản từ cơ sở dữ liệu
		    for (NhaXuatBan_DTO publisher : publishers) {
		    	tenComboBox.addItem(publisher.getMaNXB()); // Thêm tên nhà xuất bản vào combobox
		    }
		}
	 public static void loadDataIntoComboBox_maTheLoai(JComboBox<String> tenComboBox) {
		    ...
		    TheLoai_DAO theLoaiDAO = new TheLoai_DAO(connection); // Tạo một đối tượng DAO
	 ...
	 private void showImageOnLabel(String imagePath) {
		...
		private void clearFields() {
	        ...
	        comboBox_maNXB.setSelectedIndex(0);
	        ...
	        comboBox_maTheLoai1.setSelectedIndex(0);
	        comboBox_maNXB1.setSelectedIndex(0);
	        comboBox_maTacGia1.setSelectedIndex(0);
		 ...
		 public static void searchAndDisplayFromDatabase(String maSach, 
				 ...
				 donGiaTu, String donGiaDen , JTable tabel) {
			
			   ResultSet resultSet = Sach_DAO.searchAndDisplayFromDatabase(maSach, maNXB, maTheLoai,
